...
const getTripPrices = (req, res, next) => {
    ...
    for (let i = 0; i < res.locals.possibleOrders.length; i += 1) {
        let curDepartDate = dateFns.format(new Date(req.body.departureDate), 'DD/MM/YYYY'); //!initialize at first date
        const locationOrder = [
        ...
        for (let i = 0; i < locationOrder.length - 1; i++) {
            ...
            curDepartDate = dateFns.format(
                dateFns.addDays(new Date(`${temp[1]}/${temp[0]}/${temp[2]}`), locationOrder[i + 1].numDays),
                'DD/MM/YYYY',
            );
        }
...
const prepTripDataForClient = async (req, res, next) => {
    ...
    let getAllResults;
    try {
        getAllResults = await Promise.all(res.locals.tripPricePromises.map(Promise.all.bind(Promise)));
    } catch (err) {
        return next(err);
    }
