...
const TripVisualization: React.FC = (props: any): JSX.Element => {
  ...
  const focusedTripIndex = useSelector(
    (state: StoreInterface) => state.app.focusedTripIndex
  );
  ...
  useEffect(() => {
    ...
    focusedTrip.flights.forEach(flight => {
      ...
      const currentColor =
        jacobColors[Math.floor(Math.random() * jacobColors.length)];
  ...
  const flightDisplayComponents = destinationArray.map(
    (flightDestinations, i) => (
      <FlightDisplay
        flightDestinations={flightDestinations}
        color={flightColors[i]}
        key={uuid()}
      />
    )
  );
